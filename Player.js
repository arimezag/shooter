/**
 * Main character of the game. Inherits from the Character class
 * @extends Character
 */
class Player extends Character {
    /**
    * Initializes a player
    * @param game {Game} The instance of the game to which the player belongs
    */

    constructor (game) {
        const height = PLAYER_HEIGHT * game.width / 100,
            width = PLAYER_WIDTH * game.width / 100,
            x = game.width / 2 - width / 2,
            y = game.height - height,
            speed = PLAYER_SPEED,
            myImage = PLAYER_PICTURE,
            myImageDead = PLAYER_PICTURE_DEAD;

        super(game, width, height, x, y, speed, myImage, myImageDead);
        this.lives = INITIAL_LIVES;
        this.myImage = PLAYER_PICTURE;
        this.dead = false;
        this.updateLivesDisplay();
    }

    /**
     * Updates number of lives on the screen
     */
         updateLivesDisplay() {
            const livesDisplay = document.getElementById('livesli');
            if (livesDisplay) {
                livesDisplay.innerText = `Lives: ${this.lives}`;
            }
        }

    /**
     * Update the position attributes of the player and the shots based on the keys pressed
     */
    update () {
        if (!this.dead) {
            switch (this.game.keyPressed) {
            case KEY_LEFT:
                if (this.x > this.speed) {
                    this.x -= this.speed;
                }
                break;
            case KEY_RIGHT:
                if (this.x < this.game.width - this.width - this.speed) {
                    this.x += this.speed;
                }
                break;
            case KEY_SHOOT:
                this.game.shoot(this);
                break;
            }
        }
    }

    /**
     * Shoot
     */
         shoot() {
            if (!this.dead && !this.game.paused) {
                this.game.shoot(this);
                console.log();
            }
        }

    /**
     * Handle player collision
     * @param shot {Shot} Shot that hits the player
     */
     collide(shot) {
        if (this.lives > 0 && !this.dead) {
            this.lives -= 1;
            this.updateLivesDisplay(); 
            console.log();

            if (this.lives > 0) {
                this.dead = true;
                this.myImage = PLAYER_PICTURE_DEAD;
                super.collide();

                setTimeout(() => {
                    this.dead = false;
                    this.myImage = PLAYER_PICTURE;
                    console.log();
                    this.updateLivesDisplay(); 
                }, 2000);
            } else {
                super.collide();
                this.game.endGame();
                console.log();
            }
        }
    }

}